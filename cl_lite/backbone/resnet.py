...
"""
Pytorch port of the resnet used for ImageNet by PODNet.
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, last_relu=True
    ):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv1 = nn.Conv2d(
            3, nf, kernel_size=initial_kernel, stride=1, padding=1, bias=False
        )
        ...
        self.layer4 = self._make_layer(
            block, 8 * nf, layers[3], stride=2, last=True
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, last=False):
        ...
        for i in range(1, blocks):
            if i == blocks - 1 or last:
                ...
                layers.append(
                    block(self.inplanes, planes, last_relu=self.last_relu)
                )
